...
定义：
- `RoutePlannerBackend` Protocol：plan(start_latlon, end_latlon, **kwargs) -> list[(lat,lon)]
- `PlannerBackendError` Exception：规划器后端错误
...
- 接受 `vessel_mesh_path` 和 `route_config_path`
- 自动生成 `waypoints.csv`
- 解析输出 `route.json`（GeoJSON 格式）
- 转换坐标从 [lon, lat] 到 (lat, lon)
- 错误处理：失败时记录到 `polarroute_last_error.log`
...
代码修改：
```python
# 在 plan_three_routes 函数中添加规划内核参数
...
- ArcticRoute 项目：https://github.com/your-org/arcticroute
- Phase 5A 规划文档：见项目 README
