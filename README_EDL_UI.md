# EDL 风险 UI 集成指南

## 快速开始

### 启用 EDL 风险

1. 打开 Streamlit UI：`streamlit run run_ui.py`
2. 在左侧 Sidebar 中找到"风险权重"部分
3. 勾选"启用 EDL 风险（若可用）"
4. 调整"EDL 风险权重 w_edl"滑条（0.0 - 10.0）
5. 点击"规划三条方案"

### 查看 EDL 风险贡献

在"成本分解（balanced 方案）"部分，表格中会显示：
- `🧠 EDL 风险` - EDL 风险的成本贡献
- `总贡献` - 该风险分量对总成本的绝对值
- `占比` - 该风险分量占总成本的百分比

---

## 工作原理

### 数据流

```
用户启用 EDL → plan_three_routes() → build_cost_from_real_env()
  ↓
run_edl_on_features() → 生成 risk_mean
  ↓
成本场添加 edl_risk 组件
  ↓
compute_route_cost_breakdown() → 计算贡献
  ↓
UI 表格显示
```

### 成本公式

```
总成本 = 基础距离 + 海冰风险 + 波浪风险 + 冰级约束 + EDL 风险
```

其中：
```
EDL 风险 = w_edl × 期望风险分数（0..1）
```

---

## 使用场景

### 场景 1：Demo 模式

- **设置**：成本模式 = "演示冰带成本"
- **EDL 状态**：不启用（即使勾选也会被忽略）
- **用途**：快速演示，不依赖真实数据

### 场景 2：真实环境 + EDL

- **设置**：成本模式 = "真实 SIC 成本（若可用）"，启用 EDL
- **前置条件**：需要真实环境数据 + PyTorch
- **效果**：路线规划考虑多模态风险

### 场景 3：真实环境 + 无 EDL

- **设置**：成本模式 = "真实 SIC 成本（若可用）"，不启用 EDL
- **效果**：仅考虑物理风险（冰、波浪、冰级约束）

---

## 参数说明

### use_edl（启用 EDL 风险）

- **类型**：勾选框
- **默认值**：False（禁用）
- **说明**：启用基于 Evidential Deep Learning 的多模态风险评估
- **影响**：仅在"真实 SIC 成本"模式下生效

### w_edl（EDL 风险权重）

- **类型**：滑条（仅在 use_edl=True 时显示）
- **范围**：0.0 - 10.0
- **默认值**：3.0
- **说明**：调节 EDL 风险在总成本中的影响
- **建议**：
  - 0.0 - 1.0：EDL 风险影响较小
  - 1.0 - 5.0：推荐范围
  - 5.0 - 10.0：EDL 风险影响较大

---

## 降级行为

### 无 PyTorch

- **表现**：EDL 返回占位符（风险分数 = 0）
- **结果**：不报错，系统继续运行
- **日志**：`[EDL] torch not available; using fallback constant risk.`

### 无真实环境数据

- **表现**：自动回退到 demo 冰带成本
- **结果**：EDL 不被调用
- **提示**：Meta 中 `real_env_available = False`

### EDL 异常

- **表现**：捕获异常，继续使用不含 EDL 的成本
- **结果**：不报错
- **日志**：`[COST] warning: EDL risk computation failed: ...`

---

## 常见问题

### Q1：为什么启用 EDL 后成本分解中没有 EDL 风险？

**A**：可能原因：
1. 使用的是 Demo 模式（EDL 仅在真实环境模式下生效）
2. 缺少真实环境数据（SIC 或波浪数据）
3. PyTorch 不可用

**解决方案**：
- 切换到"真实 SIC 成本（若可用）"模式
- 确保有真实环境数据可用
- 安装 PyTorch：`pip install torch`

### Q2：EDL 权重应该设置多少？

**A**：根据应用场景：
- **保守**（w_edl = 1.0）：EDL 风险影响较小，主要依赖物理风险
- **平衡**（w_edl = 3.0）：推荐值，物理风险和 EDL 风险均衡
- **激进**（w_edl = 5.0+）：EDL 风险影响较大，更加谨慎

### Q3：EDL 风险和冰级约束的区别是什么？

**A**：
- **冰级约束**：基于船舶的物理能力，硬性限制
  - 软风险：冰厚在安全范围内但接近上限
  - 硬禁区：冰厚超过船舶能力，不可通行
- **EDL 风险**：基于多模态特征的概率性风险评估
  - 考虑冰、波浪、纬度、经度等多个因素
  - 提供风险分数而非硬性限制

### Q4：如何验证 EDL 是否正常工作？

**A**：
1. 启用 EDL，设置 w_edl = 5.0（较大值）
2. 规划路线，查看成本分解表
3. 应该看到"🧠 EDL 风险"行，数值 > 0
4. 禁用 EDL 重新规划，对比总成本（应该不同）

---

## 技术细节

### EDL 特征

EDL 模型使用 5 个特征：

1. **sic_norm**：海冰浓度（0..1）
2. **wave_swh_norm**：波浪有效波高（0..1，基准 10m）
3. **ice_thickness_norm**：冰厚（0..1，基准 2m）
4. **lat_norm**：纬度（0..1，60-85°N）
5. **lon_norm**：经度（0..1，-180-180°）

### EDL 输出

- **risk_mean**：期望风险分数（0..1）
- **uncertainty**：不确定性估计（≥ 0）

### 成本融合

```python
edl_cost = w_edl * risk_mean
总成本 += edl_cost
```

---

## 代码示例

### 在代码中启用 EDL

```python
from arcticroute.ui.planner_minimal import plan_three_routes
from arcticroute.core.grid import make_demo_grid

grid, land_mask = make_demo_grid()

routes, cost_fields, meta = plan_three_routes(
    grid, land_mask,
    start_lat=66.0, start_lon=5.0,
    end_lat=78.0, end_lon=150.0,
    cost_mode="real_sic_if_available",
    use_edl=True,      # 启用 EDL
    w_edl=3.0,         # 权重 3.0
)

# 查看 meta
print(meta["use_edl"])  # True
print(meta["w_edl"])    # 3.0
```

### 检查成本分解中的 EDL 风险

```python
from arcticroute.core.analysis import compute_route_cost_breakdown

cost_field = cost_fields["balanced"]
breakdown = compute_route_cost_breakdown(grid, cost_field, routes[1].coords)

# 查看 EDL 风险贡献
if "edl_risk" in breakdown.component_totals:
    edl_contribution = breakdown.component_totals["edl_risk"]
    edl_fraction = breakdown.component_fractions["edl_risk"]
    print(f"EDL 风险贡献：{edl_contribution:.2f}（占比 {edl_fraction:.2%}）")
else:
    print("EDL 风险不可用")
```

---

## 测试

### 运行 EDL 相关测试

```bash
# 运行所有 EDL 测试
pytest tests/test_cost_real_env_edl.py -v
pytest tests/test_edl_core.py -v

# 运行全部测试
pytest -v
```

### 预期结果

```
107 passed, 1 warning in 3.97s
```

---

## 后续计划

### Phase 3（建议）

1. **模型训练**：使用真实航线数据训练 EDL 模型
2. **参数优化**：调整 w_edl 默认值和特征归一化
3. **UI 增强**：添加热力图、沿程剖面等可视化
4. **不确定性显示**：在成本分解中显示 EDL 平均不确定性

---

## 相关文档

- `EDL_UI_INTEGRATION_REPORT.md` - 详细实现报告
- `IMPLEMENTATION_SUMMARY.md` - 实现总结
- `FINAL_REPORT.md` - 最终报告
- `EDL_INTEGRATION_REPORT.md` - Phase EDL-1 报告

---

## EDL 数据集构建预览（轻量）

- 使用默认配置和场景：`python -m scripts.edl_preview_dataset --scenario barents_to_chukchi --reuse`
- 配置文件：`configs/edl_dataset.yaml`；输出：`data_real/edl/training/edl_dataset_<scenario>.parquet`
- 预览内容：行/列计数、正负样本比例及关键特征的 min/max/mean；`--reuse` 直接读取现有 parquet

---

## 支持

如有问题，请查看：
1. 本文档的"常见问题"部分
2. 详细实现报告中的"降级机制"部分
3. 运行测试验证系统状态


















