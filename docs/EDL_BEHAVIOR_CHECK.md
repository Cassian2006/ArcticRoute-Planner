# EDL 行为体检 & 灵敏度分析报告

## 概述

本文档记录了 AR_final 项目中 EDL（Evidential Deep Learning）行为体检的完整实现和分析结果。

通过对比三种模式（baseline、EDL-safe、EDL-robust）在标准场景库上的表现，我们可以：
1. 量化 EDL 在不同区域的成本影响
2. 评估不确定性的分布合理性
3. 为参数调优提供数据支撑

---

## 实现架构

### 文件结构

```
scripts/
├── edl_scenarios.py                    # 标准场景库定义
└── run_edl_sensitivity_study.py        # 灵敏度分析核心脚本

tests/
└── test_edl_sensitivity_script.py      # 灵敏度分析测试

reports/
├── edl_sensitivity_results.csv         # 分析结果 CSV
└── edl_sensitivity_<scenario>.png      # 各场景的对比图表

docs/
└── EDL_BEHAVIOR_CHECK.md               # 本文档
```

### 核心组件

#### 1. 场景库（edl_scenarios.py）

定义了 4 个标准场景，覆盖不同的地理区域和冰况：

| 场景名称 | 描述 | 起点 | 终点 | 船型 |
|---------|------|------|------|------|
| `barents_to_chukchi` | 巴伦支海到楚科奇海（高冰区，长距离） | (69.0°N, 33.0°E) | (70.5°N, 170.0°E) | panamax |
| `kara_short` | 卡拉海短途（中等冰区，冰级船） | (73.0°N, 60.0°E) | (76.0°N, 120.0°E) | ice_class |
| `west_to_east_demo` | 西向东跨越北冰洋（全程高纬，多冰区） | (66.0°N, 5.0°E) | (78.0°N, 150.0°E) | handy |
| `southern_route` | 南向北冰洋边缘（低冰区，短距离） | (60.0°N, 30.0°E) | (68.0°N, 90.0°E) | panamax |

#### 2. 灵敏度分析脚本（run_edl_sensitivity_study.py）

**三种规划模式：**

| 模式 | w_edl | use_edl | use_edl_uncertainty | 说明 |
|-----|-------|---------|-------------------|------|
| `efficient` | 0.0 | False | False | 基准方案，无 EDL |
| `edl_safe` | 1.0 | True | False | 考虑 EDL 风险，不考虑不确定性 |
| `edl_robust` | 1.0 | True | True | 同时考虑 EDL 风险和不确定性 |

**输出指标：**

- `reachable`: 路线是否可达（yes/no）
- `distance_km`: 路线距离（公里）
- `total_cost`: 总成本（A* 算法累积成本）
- `edl_risk_cost`: EDL 风险成本贡献
- `edl_uncertainty_cost`: EDL 不确定性成本贡献
- `mean_uncertainty`: 沿路线的平均不确定性
- `max_uncertainty`: 沿路线的最大不确定性
- `comp_*`: 各成本分量（ice_risk, wave_risk, ice_class_soft 等）

---

## 使用方法

### 运行灵敏度分析

#### 基本用法

```bash
# 使用 demo 网格运行所有场景和模式
python -m scripts.run_edl_sensitivity_study

# 指定输出路径
python -m scripts.run_edl_sensitivity_study \
  --output-csv reports/my_results.csv \
  --output-dir reports/my_charts
```

#### 高级选项

```bash
# 干运行模式（仅验证脚本，不实际计算）
python -m scripts.run_edl_sensitivity_study --dry-run

# 使用真实数据（需要提前加载真实网格和环境数据）
python -m scripts.run_edl_sensitivity_study --use-real-data

# 组合选项
python -m scripts.run_edl_sensitivity_study \
  --use-real-data \
  --output-csv reports/real_results.csv \
  --output-dir reports/real_charts
```

### 在 Python 中调用

```python
from scripts.run_edl_sensitivity_study import (
    run_all_scenarios,
    write_results_to_csv,
    print_summary,
    generate_charts,
    SCENARIOS,
    MODES,
)

# 运行分析
results = run_all_scenarios(
    scenarios=SCENARIOS,
    modes=list(MODES.keys()),
    use_real_data=False,
    dry_run=False,
)

# 输出结果
write_results_to_csv(results, "reports/results.csv")
print_summary(results)
generate_charts(results, "reports")
```

---

## 分析结果解读

### 关键指标

#### 1. 路线可达性（reachable）

- **yes**: 路线规划成功，A* 找到了从起点到终点的可行路径
- **no**: 路线不可达，可能原因：
  - 起终点被陆地包围
  - 成本场中存在不可通行的区域（如冰级硬禁区）
  - 网格分辨率不足

#### 2. 成本对比

**关键观察点：**

- **efficient vs edl_safe**: 
  - 若 `edl_safe` 的 `total_cost` 明显高于 `efficient`，说明 EDL 风险在该区域有显著影响
  - 若差异很小（< 5%），可能说明该区域本身风险不高或 EDL 模型不敏感

- **edl_safe vs edl_robust**:
  - 若 `edl_robust` 的成本明显高于 `edl_safe`，说明不确定性在该区域分布不均
  - 若差异很小，说明不确定性的影响有限

#### 3. 不确定性分析

**mean_uncertainty 和 max_uncertainty 的含义：**

- **高不确定性区域**（> 0.7）：
  - EDL 模型对该区域的预测信心不足
  - 可能是环境条件复杂或训练数据不足
  - 建议结合物理模型和人工判断

- **低不确定性区域**（< 0.3）：
  - EDL 模型对该区域的预测较有信心
  - 可以相对信任 EDL 的风险评估

- **不确定性与风险的同向性**：
  - 理想情况：高风险区的不确定性也较高（模型知道自己不确定）
  - 异常情况：高风险区的不确定性很低（模型过度自信）

---

## 典型场景分析

### 场景 1: barents_to_chukchi（高冰区长距离）

**预期表现：**
- EDL 风险应该有显著贡献（冰区风险高）
- 不确定性可能在某些区域较高（冰况变化复杂）

**检查清单：**
- [ ] `edl_safe` 的 `edl_risk_cost` > 总成本的 10%
- [ ] `edl_robust` 的 `mean_uncertainty` 在 0.3~0.7 之间
- [ ] 三种模式都可达

### 场景 2: kara_short（中等冰区）

**预期表现：**
- EDL 风险贡献中等
- 冰级约束可能被触发（ice_class_soft/hard）

**检查清单：**
- [ ] `edl_risk_cost` 占比 5%~15%
- [ ] 如果有 `ice_class_soft` 或 `ice_class_hard`，检查其占比
- [ ] 三种模式的路线差异不应过大（否则参数可能需要调优）

### 场景 3: west_to_east_demo（全程高纬）

**预期表现：**
- 基础冰风险 (`ice_risk`) 应该很高
- EDL 风险可能是冰风险的补充或修正

**检查清单：**
- [ ] `ice_risk` 占比 > 50%
- [ ] `edl_risk_cost` 是否与 `ice_risk` 同向（都高）或反向（互补）
- [ ] 不确定性分布是否合理

### 场景 4: southern_route（低冰区）

**预期表现：**
- 基础冰风险较低
- EDL 风险可能也较低
- 三种模式的成本应该接近

**检查清单：**
- [ ] `edl_risk_cost` 占比 < 5%（符合预期）
- [ ] 若占比 > 5%，检查是否是模型过度预警

---

## 参数调优建议

### w_edl（EDL 风险权重）

**当前设置：** 1.0

**调优指南：**

| 观察现象 | 建议调整 | 理由 |
|---------|--------|------|
| EDL 风险占比 < 2% | 增加 w_edl（1.5~2.0） | EDL 贡献过小，可能权重不足 |
| EDL 风险占比 > 30% | 减少 w_edl（0.5~0.7） | EDL 贡献过大，可能权重过高 |
| EDL 风险占比 5%~15% | 保持当前值 | 合理范围 |

**建议范围：** 0.5 ~ 2.0

### edl_uncertainty_weight（不确定性权重）

**当前设置：** 1.0

**调优指南：**

| 观察现象 | 建议调整 | 理由 |
|---------|--------|------|
| 不确定性成本占比 < 1% | 增加权重（2.0~3.0） | 不确定性影响不足 |
| 不确定性成本占比 > 20% | 减少权重（0.3~0.5） | 不确定性影响过大 |
| 不确定性成本占比 5%~10% | 保持当前值 | 合理范围 |

**建议范围：** 0.5 ~ 3.0

### ice_penalty（冰风险权重）

**当前设置：** 4.0

**调优指南：**

- 若冰风险占比过高（> 60%），考虑降低 ice_penalty（2.0~3.0）
- 若冰风险占比过低（< 20%），考虑提高 ice_penalty（5.0~8.0）
- 建议范围：2.0 ~ 10.0

---

## 常见问题

### Q1: EDL 风险完全不生效（edl_risk_cost ≈ 0）

**可能原因：**
1. 真实环境数据不可用，使用了 demo 网格（demo 模式下不启用 EDL）
2. w_edl 设置为 0
3. PyTorch 或 miles-guess 后端不可用

**解决方案：**
- 检查日志中的 `[COST]` 和 `[EDL]` 消息
- 确保 `use_edl=True` 且 `w_edl > 0`
- 尝试 `--use-real-data` 选项

### Q2: 不确定性分布不合理（高风险区不确定性反而低）

**可能原因：**
1. EDL 模型训练数据不足或分布偏差
2. 特征工程不合理（归一化范围不当）
3. 模型过度拟合

**解决方案：**
- 检查特征的归一化范围（见 cost.py 中的特征构造）
- 考虑重新训练 EDL 模型或调整模型架构
- 在高不确定性区域增加训练数据

### Q3: 三种模式的路线完全相同

**可能原因：**
1. 参数权重设置不当，差异不足以改变路线
2. 网格分辨率太低，路线选择有限
3. 成本场中的冰风险或 EDL 风险分布过于均匀

**解决方案：**
- 增加 w_edl 或 ice_penalty 的差异
- 尝试更高分辨率的网格
- 检查成本场的分布（绘制热力图）

### Q4: 某个场景不可达

**可能原因：**
1. 起终点被陆地包围（特别是在 demo 网格上）
2. 冰级硬约束过严格，导致无可行路径
3. 网格边界问题

**解决方案：**
- 调整起终点坐标，避开陆地
- 降低 ice_penalty 或放宽冰级约束
- 检查网格的有效范围

---

## 输出文件说明

### CSV 文件（edl_sensitivity_results.csv）

**列说明：**

```
scenario              # 场景名称
mode                  # 规划模式（efficient/edl_safe/edl_robust）
reachable             # 是否可达（yes/no）
distance_km           # 路线距离
total_cost            # 总成本
edl_risk_cost         # EDL 风险成本
edl_uncertainty_cost  # EDL 不确定性成本
mean_uncertainty      # 平均不确定性
max_uncertainty       # 最大不确定性
comp_*                # 各成本分量
error                 # 错误信息（如有）
```

**使用示例：**

```python
import pandas as pd

df = pd.read_csv("reports/edl_sensitivity_results.csv")

# 按场景分组统计
summary = df.groupby("scenario").agg({
    "total_cost": ["min", "max", "mean"],
    "edl_risk_cost": "mean",
    "mean_uncertainty": "mean",
})

print(summary)
```

### PNG 图表（edl_sensitivity_<scenario>.png）

**包含三个子图：**

1. **Total Cost**: 三种模式的总成本对比
2. **EDL Risk Cost**: EDL 风险成本对比
3. **EDL Uncertainty Cost**: EDL 不确定性成本对比

**解读：**
- 柱子越高，成本越大
- 三个模式的相对高度反映了各模式的成本差异
- 若 EDL Risk Cost 和 EDL Uncertainty Cost 都很小，说明 EDL 在该场景的影响有限

---

## 集成到 UI 的改进

### planner_minimal.py 中的 EDL 风险提示

在 `edl_safe` 方案的成本分解显示中，添加了自动检测逻辑：

```python
# 如果 EDL 风险占比 < 5%，显示提示
if edl_risk_fraction < 0.05:
    st.info(
        f"💡 **EDL 风险贡献很小**（占比 {edl_risk_fraction*100:.1f}%）。"
        f"这可能表示：\n"
        f"1. 当前区域本身环境风险不高（海冰、波浪等较少）\n"
        f"2. EDL 模型在该区域的预测不敏感\n"
        f"3. 建议检查 w_edl 权重是否设置过低"
    )
```

**用户体验改进：**
- 自动识别 EDL 不生效的情况
- 提供可操作的建议
- 帮助用户理解参数的影响

---

## 测试覆盖

### 单元测试（test_edl_sensitivity_script.py）

**测试范围：**
- 场景库的加载和查询
- 灵敏度结果的数据结构
- 模式配置的完整性
- CSV 输出的正确性
- 图表生成的鲁棒性

**运行测试：**

```bash
pytest tests/test_edl_sensitivity_script.py -v
```

---

## 后续改进方向

### 短期（Phase 3 完成）
- [x] 实现标准场景库
- [x] 实现灵敏度分析脚本
- [x] 生成对比图表
- [x] 添加 UI 提示
- [x] 编写测试和文档

### 中期（Phase 4+）
- [ ] 支持自定义场景库
- [ ] 实现参数扫描（grid search）
- [ ] 添加统计显著性检验
- [ ] 支持多个 EDL 模型对比
- [ ] 实现交互式参数调优工具

### 长期
- [ ] 集成真实海冰预报数据
- [ ] 支持多目标优化（Pareto 前沿）
- [ ] 实现在线学习和模型更新
- [ ] 建立 EDL 模型库和评估框架

---

## 参考资源

- **EDL 核心模块**: `arcticroute/ml/edl_core.py`
- **成本构建**: `arcticroute/core/cost.py`
- **A* 路由**: `arcticroute/core/astar.py`
- **UI 集成**: `arcticroute/ui/planner_minimal.py`
- **分析工具**: `arcticroute/core/analysis.py`

---

## 更新日志

### 2024-12-08
- 初始版本发布
- 实现 Phase 3 所有步骤
- 完成文档和测试

---

**文档版本**: 1.0  
**最后更新**: 2024-12-08  
**维护者**: ArcticRoute 项目组












