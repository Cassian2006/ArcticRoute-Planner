...
def load_real_landmask_from_nc(
    ...
    优先级：
    1. 如果 nc_path 指定，尝试加载该文件
    2. 否则扫描所有候选文件（按优先级）
    3. 对每个候选文件尝试：
       a. 直接加载（形状匹配）
       b. 坐标重采样（如果有 lat/lon 坐标）
       c. 简单最近邻重采样（备选）

    返回形状为 (grid.ny, grid.nx) 的 bool 数组（True = 陆地）。
    若所有候选都失败，返回 None 并打印详细诊断信息。
    ...
    Returns:
        bool 数组（形状为 grid.shape()），或 None 如果加载失败
    """
    # 确定要尝试的文件列表
    files_to_try = []
    
    if nc_path is not None:
        files_to_try.append((Path(nc_path), "explicit"))
    else:
        # 扫描候选文件
        candidates = _scan_landmask_candidates()
        files_to_try.extend(candidates)
    
    if not files_to_try:
        print(
            f"[LANDMASK] no landmask files found. "
            f"Scanned: data_processed/env/land_mask.nc, "
            f"data_processed/newenv/land_mask_gebco.nc, "
            f"data_processed/env/land_mask_gebco.nc"
        )
        return None
    
    # 尝试每个候选文件
    for file_path, source_desc in files_to_try:
        if not file_path.exists():
            continue
        
        result = _try_load_landmask_from_file(file_path, grid)
        if result is not None:
            land_mask, meta = result
            print(
                f"[LANDMASK] successfully loaded from {source_desc}: "
                f"shape={meta['shape']}, resampled={meta['resampled']}"
            )
            return land_mask
    
    # 所有候选都失败
    print(
        f"[LANDMASK] failed to load from any candidate. "
        f"Tried: {[str(p) for p, _ in files_to_try]}. "
