...
## 任务 C3：成本侧 - 允许有坐标的密度场做重采样 ⏳ 待完成

### 需要修改的地方：
位置：`arcticroute/core/cost.py` 或 `build_cost_from_real_env` 函数

### 实现逻辑：
```python
def validate_and_resample_ais_density(ais_density_da, target_grid):
    """
    验证 AIS 密度数据并进行必要的重采样
    
    规则：
    1. 如果 AIS DataArray 带有 latitude/longitude 坐标：
       - 允许使用 xarray.interp 重采样到目标网格
       - 精度足够用于成本计算
    ...
    if hasattr(ais_density_da, 'coords'):
        if 'latitude' in ais_density_da.coords and 'longitude' in ais_density_da.coords:
            ...
            # 无坐标，拒绝
            raise ValueError(
                f"该密度文件为 demo 网格产物（{ais_density_da.shape}），"
                f"请生成 {target_grid.shape()} 版本"
            )
```
...
| A | ✅ 完成 | AIS 状态管理已实现 |
| B | ✅ 完成 | 无重复管线代码 |
| C1 | ⏳ 待完成 | 需要在侧边栏添加网格变化检测 |
| C2 | ⏳ 待完成 | 需要修改 AIS 预处理脚本 |
| C3 | ⏳ 待完成 | 需要修改成本计算逻辑 |
